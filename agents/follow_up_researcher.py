...
def follow_up_researcher_node(state):
    ...
    for item in state["knowledge_gaps"]:
        ...
        # Use a shorter, targeted query for Tavily
        search_query = f"{question} {' '.join(weaknesses)}"

        # Tavily max query length protection
...
- Include quantitative evidence only if it appears in the evidence.
